...
namespace TruckMove.API.DAL.Repositories.JobRepositories
{
    public class TaskRepository : ITaskRepository
    {
        ...
        {
            return await _context.Set<Accommodation>().Where(x => x.Job.IsActive == true &&
        ...
        {
            return await _context.Set<PermitsAndPlate>().Where(x =>x.Job.IsActive==true && 
                                                                   x.Job.Status < (int)JobStatusEnum.QADone &&
                                                                   x.OrganizeNow == false && 
                                                                   x.Assignee == userId && 
                                                                   x.Status != (int)TaskStatusEnum.Completed &&
        ...
        {
            return await _context.Set<PublicTransport>().Where(x => x.Job.IsActive == true &&
                                                                   x.Job.Status < (int)JobStatusEnum.QADone &&
                                                                   x.OrganizeNow == false &&
                                                                   x.Assignee == userId &&
                                                                   x.Status != (int)TaskStatusEnum.Completed &&
        ...
        {
            return await _context.Set<Purchase>().Where(x => x.Job.IsActive == true &&
                                                                   x.Job.Status < (int)JobStatusEnum.QADone &&
                                                                   x.OrganizeNow == false &&
                                                                   x.Assignee == userId &&
                                                                   x.Status != (int)TaskStatusEnum.Completed &&
